...
import { clearAlertHistory, createPerformanceScope, disablePerformanceAlerts, disablePerformanceMonitoring, enablePerformanceAlerts, enablePerformanceMonitoring, getAlertStats, getAllMetricNames, getAverageMetric, getMetricData, getPerformanceAlerts, getPerformanceReport, isPerformanceMonitoringEnabled, measureAsync, measurePerformance, onPerformanceAlert, recordMetric, resetAlertManager, resetMetric, resetMetrics, setAlertManager, setAlertThreshold, wrapAsync, wrapSync, } from '../../../src/utils/performance/integration';
setAlertManager({
    check: () => { },
    clear: () => { },
    onAlert: () => { },
...
describe('performance integration', () => {
    ...
    it('should wrap async functions and return result', async () => {
        const wrapped = wrapAsync('async_metric', async (value) => value + 1);
        const result = await wrapped(1);
        expect(result).toBe(2);
    });
    it('should wrap sync functions and return result', () => {
        const wrapped = wrapSync('sync_metric', (value) => value * 2);
    ...
    it('should handle errors in wrapAsync', async () => {
        const wrapped = wrapAsync('error_async', async () => {
    ...
    it('should handle errors in wrapSync', () => {
        const wrapped = wrapSync('error_sync', () => {
    ...
    it('should use measurePerformance decorator via descriptor', async () => {
        ...
        const decoratedDescriptor = measurePerformance('decorator_metric')({}, 'testMethod', descriptor);
    ...
    it('should export and call alert manager functions', () => {
        ...
        setAlertManager(mockAlertManager);
        resetAlertManager();
        // 验证函数存在且可调用
