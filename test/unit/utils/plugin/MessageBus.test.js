...
import { MessageBus, } from '../../../../src/utils/plugin';
describe('MessageBus', () => {
    let messageBus;
    beforeEach(() => {
    ...
    describe('subscribe', () => {
        ...
        it('should enforce max subscriptions limit', () => {
            const config = { maxSubscriptions: 2 };
        ...
        it('should handle zero maxSubscriptions (no limit)', () => {
            const config = { maxSubscriptions: 0 };
    ...
    describe('publish', () => {
        ...
        it('should handle async handlers', async () => {
            const handler = jest.fn(async (msg) => {
    ...
    describe('filter', () => {
        it('should filter messages based on filter function', async () => {
            ...
            const handler2 = jest.fn();
            messageBus.subscribe('test:type', handler1, {
                filter: (msg) => msg.payload.value > 5,
            });
            messageBus.subscribe('test:type', handler2);
    ...
    describe('timeout handling', () => {
        it('should timeout handler if configured', async () => {
            const config = { handlerTimeout: 100 };
            const timedOutBus = new MessageBus(config);
            const handler = jest.fn(() => new Promise((resolve) => setTimeout(resolve, 200)));
            const errorHandler = jest.fn((_error, _message) => { });
    ...
    describe('enableLogging: true', () => {
        ...
        it('should log subscribe action', () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('Subscribed to "test"'));
        });
        it('should log unsubscribe action', () => {
            const id = loggingMessageBus.subscribe('test', () => { });
            mockLogger.debug.mockClear();
            loggingMessageBus.unsubscribe(id);
            expect(mockLogger.debug).toHaveBeenCalledWith(expect.stringContaining('Unsubscribed from "test"'));
        });
        it('should log warning for subscription not found', () => {
            loggingMessageBus.unsubscribe('non-existent-id');
            expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('Subscription not found: non-existent-id'));
        });
        it('should log unsubscribeAll action', () => {
            ...
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Unsubscribed 2 subscriptions from "test"'));
        });
        it('should log clear action', () => {
            ...
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Cleared 2 subscriptions'));
        });
        it('should log publish message', () => {
            loggingMessageBus.subscribe('test', () => { });
            consoleLogSpy.mockClear();
            loggingMessageBus.publish('test', { data: 'test' });
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('Publishing message type "test"'));
        });
        it('should log no subscribers message', () => {
            consoleLogSpy.mockClear();
            loggingMessageBus.publish('test', { data: 'test' });
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('No subscribers for message type "test"'));
        });
